...
from datasets.scannet.ray_utils import get_ray_directions, get_rays
from datasets.scannet.geo_utils import (
    bbox_intersection_batch,
    center_pose_from_avg,
    observe_angle_distance,
)
from datasets.scannet.image_utils import compute_distance_transfrom_weights, rebalance_mask
...
class GenericDataset(Dataset):
    ...
    def read_bbox_info(self):
        ...
        self.instance_bboxes = scene_bbox
        self.bbox_bounds = []
        for b in scene_bbox:
            # if b[6] == self.conf["val_instance_id"]:
            length = np.array([b[3], b[4], b[5]]) * 0.5
            center = np.array([b[0], b[1], b[2]])
            self.bbox_bounds.append(np.array([center - length, center + length]))
    ...
    def read_frame_data(self, frame, instance_id, read_instance_only=False):
        ...
        if self.conf["use_instance_mask"] and instance_id != 0:
            ...
            instance_mask_weight = self.transform(instance_mask_weight).view(-1)
        ...
        if ray_in_bbox:
            ...
            bbox_mask, batch_near, batch_far = bbox_intersection_batch(
                self.bbox_bounds[instance_id-1], rays_o_bbox, rays_d_bbox
            )
